# Step by step guide on using MolCrafts' DevContainer

Using MolCrafts' DevContainer environment involves a series of steps that will help you set up a development environment that is isolated and replicable. Below, we'll guide you through each step to make this process as smooth as possible:
...
## Step 5: Clone MolCrafts

1. Open a terminal or command prompt.
2. Use the following command to clone the MolCrafts repository:
   ```
   git clone https://github.com/MolCrafts/molcrafts
   ```
...
3. Once the build process completes, you'll have a fully configured MolCrafts development environment in a container.
4. The next time you open the same dev container, it will be much faster, as it does not require building the image again.

You are now all set to start developing with MolCrafts in a DevContainer environment. This setup ensures you have a consistent and isolated development environment for your MolCrafts projects.

## Step 8: Build MolCrafts

To build molcrafts from source, simply run:
   ```
...
Note that only contents in the `molcrafts` directory are saved to disk. This directory is mounted to the docker image, while other contents in the docker image are all temporary, and will be lost if docker restarts the image or the server reboots.
